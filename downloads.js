(() => {
    ...
    const copy = {
        en: {
            ...
            "catalog.kicker": "PUMPSOUND ARCHIVE", "catalog.title": "DOWNLOADS", "catalog.lede": "The complete PUMPSOUND catalogue. New YouTube releases appear here automatically.", "catalog.searchLabel": "Search tracks", "catalog.searchPlaceholder": "SEARCH TRACKS", "catalog.available": "YOUTUBE RELEASES", "catalog.empty": "No releases match your search.",
        ...
        pl: {
            ...
            "catalog.kicker": "ARCHIWUM PUMPSOUND", "catalog.title": "POBIERANIE", "catalog.lede": "Pełny katalog PUMPSOUND. Nowe premiery YouTube pojawiają się tu automatycznie.", "catalog.searchLabel": "Szukaj utworów", "catalog.searchPlaceholder": "SZUKAJ UTWORÓW", "catalog.available": "PREMIERY YOUTUBE", "catalog.empty": "Brak utworów pasujących do wyszukiwania.",
    ...
    async function getCatalogue() {
        const [youtubeResponse, configResponse] = await Promise.all([fetch("/api/youtube"), fetch("/data/downloads.json", { cache: "no-cache" })]);
        ...
        const tracks = (Array.isArray(youtube.videos) ? youtube.videos : []).map((video) => {
            const download = downloadsByVideoId.get(video.id) || {};
            return { id: video.id, slug: download.slug || slugify(video.title, video.id), title: video.title || "PUMPSOUND RELEASE", artist: "PUMPSOUND", thumbnail: video.thumbnail || "", uploadDate: video.publishedAt || "", views: video.views, youtubeUrl: video.url || `https://www.youtube.com/watch?v=${encodeURIComponent(video.id)}`, displayArtist: download.displayArtist || null, displayTitle: download.displayTitle || null, displayVersion: download.displayVersion || null, fileKey: download.fileKey || null, format: download.format || null, bitrate: download.bitrate || null, fileSize: download.fileSize || null, duration: download.duration || null };
        });
    ...
    async function renderCurrentPage() {
        ...
        catch (error) { const target = document.querySelector("#download-list, #download-track"); if (target) target.innerHTML = `<p class="download-error">${t("track.notFound")}</p>`; console.error("Download catalogue error:", error); }
    }
